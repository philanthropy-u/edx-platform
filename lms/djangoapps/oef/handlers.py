...
def update_oef_prompts(instance, **kwargs):
    this_oef_prompt = OrganizationOefUpdatePrompt.objects.filter(org_id=instance.org_id).first()
    ...
    if this_oef_prompt:
        this_oef_prompt.responsible_user = responsible_user
        this_oef_prompt.latest_finish_date = latest_finish_date
        this_oef_prompt.year = year
        this_oef_prompt.save()
    else:
