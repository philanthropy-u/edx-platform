...
from openedx.features.courseware import constants
...
class PrePostAssessmentMixin(ModuleStoreTestCase):
    ...
    def create_course_and_user(self):
        ...
        self.add_pre_post_assessment_grading_policy()
    ...
    def get_pre_post_assessment_api_response(self, chapter_id):
        """Method to create url and call api"""
        url = reverse(
            'courseware:pre_post_assessments_score',
            kwargs={
    ...
    def add_pre_post_assessment_grading_policy(self):
...
class PrePostAssessmentScoreForUnEnrolledUserTest(PrePostAssessmentMixin, APITestCase):
    ...
    def test_un_enrolled_user_request(self):
        ...
        response = self.get_pre_post_assessment_api_response(self.create_chapter().location.block_id)
...
class PrePostAssessmentScoreForEnrolledUserTest(PrePostAssessmentMixin, APITestCase):
    ...
    def test_invalid_chapter_id(self):
        ...
        response = self.get_pre_post_assessment_api_response('invalid_chapter_id')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_no_pre_post_assessment_in_chapter(self):
        ...
        response = self.get_pre_post_assessment_api_response(chapter.location.block_id)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_pre_post_assessment_chapter(self):
