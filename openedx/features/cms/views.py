...
def latest_course_reruns(courses):
    ...
    for course in courses:
        ...
        is_course_parent_course = bool(CourseRerunState.objects.filter(source_course_key=course.id))
        ...
        if is_course_parent_course:
            sibling_re_runs = CourseRerunState.objects.filter(
                source_course_key=course.id).order_by(
                '-created_time').all()
        else:
            sibling_re_runs = CourseRerunState.objects.filter(
                source_course_key=course_rerun.source_course_key).order_by(
                '-created_time').all()
