...
class StudioAutomationTestCase(CourseTestCase):
    """
    Class for test cases related to course re-run (course automation, in general)
    """
    def setUp(self):
        super(StudioAutomationTestCase, self).setUp()
        # create source course
        self.source_course = create_source_course(self.store, self.user,
    ...
    def test_set_rerun_course_dates_with_load_factor(self, mock_set_rerun_schedule_dates,
        ...
        level of component
        """
        source_courses = create_large_course(self.store, 2, datetime(2019, 4, 1, tzinfo=UTC))
        rerun_courses = create_large_course(self.store, 2, datetime(2019, 4, 1, tzinfo=UTC))
    ...
    def test_component_update_successful(self):
        ...
        """
        courses = create_large_course(self.store, 1)
    ...
    def tearDown(self):
        super(StudioAutomationTestCase, self).tearDown()
